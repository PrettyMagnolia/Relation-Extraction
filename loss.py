...
class Two_Headed_Loss(nn.Module):
    ...
    '''
    def __init__(self, lm_ignore_idx, use_logits=False, normalize=False, sample_size=8):
    ...
    def blank_loss(self, blank_logits, blank_labels):
        ...
        return torch.cat([pos_logits, neg_logits], dim=0), blank_labels_
    ...
    def forward(self, lm_logits, blank_logits, lm_labels, blank_labels):
        ...
        black_log, black_lab= torch.FloatTensor([]).cuda(), torch.FloatTensor([]).cuda()
        for i in range(0, len(blank_logits), self.sample_size):
            logits, label = self.blank_loss(blank_logits[i:i+8], blank_labels[i:i+8])            
            black_log = torch.cat([black_log, logits], dim=0)
            black_lab = torch.cat([black_lab, label], dim=0)
    
        blank_loss = self.BCE_criterion(black_log, black_lab)
