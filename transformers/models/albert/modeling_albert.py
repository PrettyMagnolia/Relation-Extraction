...
class AlbertModel(AlbertPreTrainedModel):
    ...
    def __init__(self, config: AlbertConfig, add_pooling_layer: bool = True, n_classes: int = None):
        super().__init__(config)
        ...
        if n_classes:
            self.classifier = nn.Linear(config.hidden_size * 2, n_classes)
    ...
    def forward(
        ...
        e1_e2_start: Optional[torch.LongTensor] = None
        ...
        e1_start_idx = e1_e2_start[:, 0]  # (batch_size,)
        e2_start_idx = e1_e2_start[:, 1]  # (batch_size,)

        e1_vectors = sequence_output[torch.arange(sequence_output.size(0)), e1_start_idx, :]  # (batch_size, hidden_size)
        e2_vectors = sequence_output[torch.arange(sequence_output.size(0)), e2_start_idx, :]  # (batch_size, hidden_size)

        # Concatenate the vectors of e1 and e2
        v1v2 = torch.cat((e1_vectors, e2_vectors), dim=1)  # (batch_size, 2 * hidden_size)
